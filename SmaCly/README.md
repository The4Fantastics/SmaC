...
* **Transform Blockly to Solidity**. The "Transform Blockly to Solidity" button transforms the block model built on the tool canvas to Solidity code, to be displayed in the left area of the tool.
* **Save Solidity Code**. The "Save Solidity Code" button downloads the Solidity code generated from the block model built on the tool's canvas. To download said file, it is necessary to have previously generated the Solidity code.
